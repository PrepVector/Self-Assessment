...
Playwright Renderer: Programmatically transforms the AI's Markdown report into a beautifully styled, professional PDF document.

4. Frictionless Intake & Email Dispatch

Zero-Dependency Persistence: Fully relies on standard Python CSV handling for instant, lightweight data persistence—no MongoDB or external database required.

Resend Integration: Captures candidate emails post-assessment and uses the Resend API to securely queue and dispatch final PDF reports directly to their inboxes.

🛠️ Tech Stack

Frontend: React (Vite), Tailwind CSS, React Markdown, React Confetti

Backend: FastAPI (Python 3.12+) served via Uvicorn

AI Engine: Google GenAI SDK (Gemini Models), Groq (Fallback)
...
FastAPI is the web framework we use to build the backend API, but it requires an ASGI (Asynchronous Server Gateway Interface) server to actually run and listen for web traffic. We use Uvicorn for this purpose.
...
Create a .env file inside the backend directory. You will need to provision three API keys:

Gemini: Get this from Google AI Studio.

Resend: Create a free account at Resend to get an API key. (Note: On the free tier, you can only send emails to the address you registered with).

Groq: Get this from the Groq Console.
...
The --reload flag ensures the server automatically restarts when you make code changes. The backend will run on http://127.0.0.1:8000.
...
Vite will provide a local URL (e.g., http://localhost:5173) to view the application in your browser.

📚 Managing the Question Bank

The application uses an AI-driven script (build_question_bank.py) to generate the underlying 147-question pool.

Best Practice: To conserve API quota, prevent rate limits, and reduce execution time, it is highly advisable to generate only one section at a time. Open build_question_bank.py, comment out the completed sections inside the SECTIONS array, and leave only your target section uncommented.
